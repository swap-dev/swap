...
### On Linux for Android (using docker):
        ...
        docker build -f utils/build_scripts/android32.Dockerfile -t swap-android .
        # Build image (for ARM 64-bit)
        docker build -f utils/build_scripts/android64.Dockerfile -t swap-android .
        # Create container
        docker create -it --name swap-android swap-android bash
        # Get binaries
        docker cp swap-android:/src/build/release/bin .
...
To list all available options, run `./bin/swapd --help`.  Options can be
...
To run in background:
    ...
    ./bin/swapd --log-file swapd.log --detach

To run as a systemd service, copy
[swapd.service](utils/systemd/swapd.service) to `/etc/systemd/system/` and
[swapd.conf](utils/conf/swapd.conf) to `/etc/`. The [example
service](utils/systemd/swapd.service) assumes that the user `monero` exists
and its home is the data directory specified in the [example
config](utils/conf/swapd.conf).

If you're on Mac, you may need to add the `--max-concurrency 1` option to
swap-wallet-cli, and possibly swapd, if you get crashes refreshing.
...
* `--p2p-bind-ip 127.0.0.1` on the command line or `p2p-bind-ip=127.0.0.1` in
  swapd.conf to disable listening for connections on external interfaces.
* `--no-igd` on the command line or `no-igd=1` in swapd.conf to disable IGD
  (UPnP port forwarding negotiation), which is pointless with Tor.
* `DNS_PUBLIC=tcp` or `DNS_PUBLIC=tcp://x.x.x.x` where x.x.x.x is the IP of the
  desired DNS server, for DNS requests to go over TCP, so that they are routed
  through Tor. When IP is not specified, swapd uses the default list of
  servers defined in [src/common/dns_utils.cpp](src/common/dns_utils.cpp).
* `TORSOCKS_ALLOW_INBOUND=1` to tell torsocks to allow swapd to bind to interfaces
...
* Do NOT pass `--detach` when running through torsocks with systemd, (see
  [utils/systemd/swapd.service](utils/systemd/swapd.service) for details).
* If you use the wallet with a Tor daemon via the loopback IP (eg, 127.0.0.1:9050),
  then use `--untrusted-daemon` unless it is your own hidden service.

Example command line to start swapd through Tor:

    DNS_PUBLIC=tcp torsocks swapd --p2p-bind-ip 127.0.0.1 --no-igd
...
allow inbound connections. Full example:
    ...
    DNS_PUBLIC=tcp torsocks ./swapd --p2p-bind-ip 127.0.0.1 --no-igd --rpc-bind-ip 127.0.0.1 \
...
When it terminates with an output along the lines of "Segmentation fault (core dumped)", there should be a core dump file in the same directory as swapd. It may be named just `core`, or `core.xxxx` with numbers appended.

You can now analyse this core dump with `gdb` as follows:

`gdb /path/to/swapd /path/to/dumpfile`

Print the stack trace with `bt`

* To run monero within gdb:

Type `gdb /path/to/swapd`

Pass command-line options with `--args` followed by the relevant arguments

Type `run` to run swapd
...
Install valgrind and run as `valgrind /path/to/swapd`. It will be very slow.
