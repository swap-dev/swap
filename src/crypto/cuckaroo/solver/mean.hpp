...
public:
  ...
  void genUVnodes(const u32 id) {
    //u64 rdtsc0, rdtsc1;
    const u32 NEBS = 1 * EDGE_BLOCK_SIZE;
    alignas(8) u64 buf[NEBS];
    ...
    for (u32 my = starty; my < endy; my++, endedge0 += NYZ) {
      ...
      for (; edge0 < endedge0; edge0 += NEBS) {
        ...
        for (u32 e = 0; e < NEBS; e += 1) {
          shs.hash24(edge0 + e);
          buf[e] = shs.xor_lanes();
        }
        for (u32 ns = 0; ns < 1; ns++) {
          const u64 buflast = buf[NEBS - 1 + ns];
          buf[NEBS - 1 + ns] = 0;
          for (u32 e = 0; e < NEBS; e += 1) {
...
public:
  ...
  void setheadernonce(char* const headernonce, const u32 len) {
...
// write      UYYZZZ'    VYYZZ'   within VX partition
  ...
  void *matchUnodes(match_ctx *mc) {
    //u64 rdtsc0, rdtsc1;
    const u32 NEBS = EDGE_BLOCK_SIZE;
    alignas(8) u64 buf[NEBS];
            ...
            if (uxymap[node0 >> ZBITS]) {
              for (u32 j = 0; j < PROOFSIZE; j++) {
                if (cycleus[j] == node0 && cyclevs[j] == node1) {
                  sols[sols.size()-PROOFSIZE + j] = edge0 + ns * EDGE_BLOCK_SIZE + e/1;
                }
              }
            }
          }
