...
int Cuckaroo29S::hash(const void* in, size_t len, uint64_t nonce8, uint32_t nonce, uint32_t *edges,void* out)
{
	...
	header[len+11] = nonce & 0xff;
	header[len+10] = (nonce >> 8 ) & 0xff ;
	header[len+9] = (nonce >> 16 ) & 0xff ;
	header[len+8] = (nonce >> 24 ) & 0xff ;
	header[len+7] = nonce8 & 0xff;
	header[len+6] = (nonce8 >> 8 ) & 0xff ;
	header[len+5] = (nonce8 >> 16 ) & 0xff ;
	header[len+4] = (nonce8 >> 24 ) & 0xff ;
	header[len+3] = (nonce8 >> 32 ) & 0xff ;
	header[len+2] = (nonce8 >> 40 ) & 0xff ;
	header[len+1] = (nonce8 >> 48 ) & 0xff ;
	header[len]   = (nonce8 >> 56 ) & 0xff ;
	
	setheader((const unsigned char*)header,len+12,&keys);
